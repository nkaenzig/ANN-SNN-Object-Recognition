...
def plot_spiketrains(layer, dt, path=None):
    ...
    if path is not None:
        ...
        plt.savefig(os.path.join(os.path.dirname(path), '{}.png'.format(layer[1])), bbox_inches='tight')
        #plt.savefig(os.path.join(os.path.dirname(path), '{}.eps'.format(layer[1])), bbox_inches='tight', format='eps')
        #plt.savefig(os.path.join(path, filename), bbox_inches='tight')
    else:
        plt.show()
    plt.close()
