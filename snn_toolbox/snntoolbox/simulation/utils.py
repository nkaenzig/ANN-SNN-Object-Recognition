...
class AbstractSNN:
    ...
    def run(self, x_test=None, y_test=None, dataflow=None, **kwargs):
        ...
        ann_guesses_d = []  # Filled up with guessed classes of all test samples.
        ...
        for batch_idx in range(num_batches):
            ...
            print("Moving accuracy of ANN (top-1, top-{}): {:.2%}, {:.2%}."
                  "\n".format(self.top_k, 1 - self.top1err_ann,
                              1 - self.top5err_ann))
            ann_guesses_d.append(np.argmax(self.parsed_model.predict(x_b_l)))
        ...
        ann_count = np.zeros(self.num_classes)
        ann_match = np.zeros(self.num_classes)
        for gt, p in zip(truth_d, ann_guesses_d):
            ann_count[gt] += 1
            if gt == p:
                ann_match[gt] += 1
        ann_avg_acc = np.mean(np.true_divide(ann_match, ann_count))
        ann_top1acc_total = np.mean(np.array(truth_d) == np.array(ann_guesses_d))

        # Print final result.
        print("Simulation finished.\n\n")
        ss = '' if self.config.getint('simulation', 'num_to_test') == 1 else 's'
        print("SNN Total accuracy: {:.2%} on {} test sample{}.\n\n".format(
            top1acc_total, len(guesses_d), ss))
        print("ANN Total accuracy: {:.2%} on {} test sample{}.\n\n".format(
            ann_top1acc_total, len(guesses_d), ss))
        print("SNN Accuracy averaged over classes: {:.2%}".format(avg_acc))
        print("ANN Accuracy averaged over classes: {:.2%}".format(ann_avg_acc))

        class_accuracies = np.array(match) / np.array(count, dtype=float)
        print("SNN CLASS ACCURACIES N L C R: {} {} {} {}".format(class_accuracies[0], class_accuracies[1], class_accuracies[2], class_accuracies[3]))
        ann_class_accuracies = np.array(ann_match) / np.array(ann_count, dtype=float)
        print("ANN CLASS ACCURACIES N L C R: {} {} {} {}".format(ann_class_accuracies[0], ann_class_accuracies[1], ann_class_accuracies[2], ann_class_accuracies[3]))
