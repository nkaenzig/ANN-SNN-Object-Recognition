...
def extract_batch(xaddr, yaddr, timestamps, pol, frame_gen_method, batch_size,
    ...
    for sample_idx in range(batch_size):
        ...
        # NK
        last_x =xaddr[event_idxs][0]
        last_y =yaddr[event_idxs][0]
        last_t =timestamps[event_idxs][0]
        single_mode = False

        for x, y, p, t in zip(xaddr[event_idxs], yaddr[event_idxs],
                           pol[event_idxs], timestamps[event_idxs]):
            ...
            # someplace else. Here we output only 1d lists.
                if single_mode:
                    if t!=last_t or x!=last_x or y!=last_y:
                        xaddr_sub.append(x)
                        yaddr_sub.append(y)
                    else:
                        print("TIME CLUSTER AVOIDED")
                else:
                    xaddr_sub.append(x)
                    yaddr_sub.append(y)
